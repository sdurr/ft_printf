...
/*   Updated: 2015/02/03 11:16:24 by sdurr            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		ft_point_space(char *s, int *i, char **aff, size_t stop)
{
	...
	test = 0;
	j = 0;
	if (s[i[6]--] == '.' && (tmp = ft_strnew(13)))
	{
		test = 1;
		while (s[i[6]] >= '0' && s[i[6]] <= '9')
			tmp[j++] = s[i[6]--];
		if ((j = ft_atoi(ft_revers(tmp))) == 15 && stop == 2)
			j = 13;
...
static int		ft_return_s_maj(char *s, int *c, wchar_t *s1, char *tmp)
{
	...
	while (s1[j] && (c[0] += ft_calc_and_print_wchar(s1[j++], tmp)))
		if (j >= c[3] && c[4] > 0 && c[1] == 1)
			return (c[0]);
	return (ft_suite_s_maj(c, 1, s));
}

int				ft_print_s_maj(wchar_t *s1, char *s, int i, char *tmp)
{
	int			c[7];
	...
	if (c[4] == 0 && s[i] == '.')
		return (c[1]);
	return (ft_return_s_maj(s, c, s1, tmp));
}
