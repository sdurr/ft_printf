...
/*   Updated: 2015/02/03 09:46:30 by sdurr            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_suite_point_space(size_t j, int *i, char **aff, char *s1)
{
	if (i[0] == 0)
		while (j--)
			*aff = ft_strjoin(*aff, " ");
...
static int		ft_point_space(char *s1, char *s, int *i, char **aff)
{
	...
	if (s[i[2]--] == '.' && (tmp = ft_strnew(13)))
	{
		...
		test = j;
		if (s1)
			ft_suite_point_space(j, i, aff, s1);
		else
			while (j--)
...
static int		ft_suite(size_t j, char **aff, char *s, int i)
{
	while (j--)
